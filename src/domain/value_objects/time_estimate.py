...
class TimeEstimate:
    ...
    def __post_init__(self):
        ...
        if not (self.optimistic_hours <= self.realistic_hours <= self.pessimistic_hours):
            raise ValueError("Time estimates must follow: optimistic <= realistic <= pessimistic")
    ...
    def expected_hours(self) -> float:
        ...
        return (self.optimistic_hours + 4 * self.realistic_hours + self.pessimistic_hours) / 6
    ...
    @classmethod
    def from_hours(cls, optimistic: float, realistic: float, pessimistic: float) -> "TimeEstimate":
        """Create time estimate from hour values."""
        return cls(
            optimistic_hours=optimistic, realistic_hours=realistic, pessimistic_hours=pessimistic
        )
