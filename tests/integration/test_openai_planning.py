...
class TestOpenAIPlanningIntegration:
    ...
    async def test_plan_real_project_with_openai(self):
        ...
        for task in response.tasks:
            ...
            for dep_id in task.dependencies:
                assert any(
                    t.id == dep_id for t in response.tasks
        ...
        for i, task in enumerate(response.tasks, 1):
            deps = (
                f" (depends on: {', '.join(map(str, task.dependencies))})"
    ...
    async def test_plan_technical_project_with_details(self):
        ...
        total_hours = sum(
            task.time_estimate.expected_hours for task in response.tasks if task.time_estimate
        )
        assert (
            40 <= total_hours <= 500
